...
Six screens represent functionality that is not wired up yet, so the eventual experience can be seen and demonstrated before the licence and the integrations exist:

| Screen | Shows | Waiting on |
|---|---|---|
| `/watch/preview/` | The app row, opening directly | Pro:Centric Cloud (1c) |
| `/cast/preview/` | The pairing screen | Pro:Centric Cloud (1c) |
| `/eat/preview/` | Book a table, send to phone | Phase 3 |
| `/stay/late-checkout/` | Availability, fee, request button | MEWS (2) + Phase 3 |
| `/stay/your-bill/` | Itemised bill, settled | MEWS (2) |
| `/stay/message-us/` | QR handoff, one-press requests | Contact route + Phase 3 |

**Every control on a preview screen responds when pressed.** It raises a toast saying what would have happened, and records the press to `sessionStorage`, which `/debug/` reads back. This is deliberate: a preview whose buttons do nothing teaches you nothing about whether the press was received, and on a television there are no devtools to check. Press a button in the room, open `/debug/`, read what fired.

Wiring a control up means removing its `data-mock` attribute and attaching a real handler. Nothing else changes.

**Honesty rules applied to these screens:**

- Every one carries a gorse `PREVIEW` flag naming what it waits on
- The bill's figures are deliberately zeroed so they can never be mistaken for a real bill
- QR slots are dashed placeholders, never a scannable code. The real pairing code is generated by the television's own Screen Share app; a fake QR a guest scans and gets nothing from is worse than none
- "Room appears free" on the late checkout screen is marked illustrative

**Before go-live**, the three "See what this becomes" cards on the watch, cast and eat sections should be removed &mdash; they are for demonstrating to David and Samantha, not for guests.

**One decision that is David's, not technical:** §11.2 of the scope pack limits the TV layer to name and stay dates. Whether billing detail belongs on a screen in a room at all is worth deciding before the bill screen is wired to MEWS.
